...
**
** $Log: elf2cfetbl.c  $
** Revision 1.10 2016/03/25 abrown
** Added SC table construction when special symbols detected
...
#define SUCCESS       (0)
#define FAILED        (1)
#define SIZEMISMATCH  (2)

/* Table symbol length, 
 must match CFE_TBL_FileDef_t ObjectName, cfe_tbl_filedef.h */
#define CFETBLSYMLENGTH (64) 

/* For SC Table special handling */
#define SC_ELF2TBL_HOOK ("SCCommandList")
#define SC_MAX_CMDS (256U)
...
typedef struct ATS_List_Entry {
	uint32 TimeTag; /* absolute time (sec) for command */
	char cmdSymbolName[255]; /* symbol name of the command data, above */
	uint16 byteSize; /* size of the entire message (bytes) */
} ATS_List_Entry;

/* (end) For SC Table special handling */
...
int32 LocateAndReadUserObjectI(CFE_TBL_FileDef_t *localDef, boolean *DataIsAllZeros, int32 *objSymbolIndex);
int32 LocateAndReadUserObject(CFE_TBL_FileDef_t *localDef, boolean *DataIsAllZeros);
int32 LocateAndReadUserObjectIndex(char * objName, boolean doWarn);
int32 GetBytesForSymbol(CFE_TBL_FileDef_t *dataDescrip, uint8 *data);
uint8 CCSDS_ComputeCheckSum (CCSDS_CmdPkt_t *PktPtr);
void Write_CCSDS_Checksum(CCSDS_CmdPkt_t *PktPtr, uint8 cksum);
void CCSDS_LoadCheckSum (CCSDS_CmdPkt_t *PktPtr);
int32 GenerateSCTable(void);
...
int32 UserObjSymbolIndex = -1; /* the table data */
...
int main(int argc, char *argv[])
{
    ...
    Status = LocateAndReadUserObjectI(&TblFileDef, &TableDataIsAllZeros, &UserObjSymbolIndex);
    if (Status == FAILED)  /* less-strict checking to preserve old behavior */
    {
        FreeMemoryAllocations();
        return Status;
    }
...
/**
* Locates the data in the symbol table and sets the global SrcFileDesc file pointer
* to the correct location for that data.  (Note, this depends upon several globals
* being set.  Only call this after the object file has been parsed.)
*
* Args: localDef = the information on the expected name and size.
*                  Required fields: ObjectName, ObjectSize
*       DataIsAllZeros = set to T if the found symbol is in the .bss section,
*                        set to F if there is data for that symbol
*
* Returns SUCCESS , FAILED, or SIZEMISMATCH (the given localDef does not match the
*         reported (nonzero) Elf symbol length).
*/
int32 LocateAndReadUserObject(CFE_TBL_FileDef_t *localDef, boolean *DataIsAllZeros)
{
	int32 objSymbolIndex = -1;
	return LocateAndReadUserObjectI(localDef, DataIsAllZeros, &objSymbolIndex);
}
...
* to the correct location for that data.  (Note, this depends upon several globals
* being set.  Only call this after the object file has been parsed.)
* This variant will return the found symbol table index.
*
* Args: localDef = the information on the expected name and size.
*                  Required fields: ObjectName, ObjectSize
*       DataIsAllZeros = set to T if the found symbol is in the .bss section,
*                        set to F if there is data for that symbol
*       objSymbolIndex = The symbol table index when the data was located.
*
* Returns SUCCESS , FAILED, or SIZEMISMATCH (the given localDef does not match the
*         reported (nonzero) Elf symbol length).
*/
int32 LocateAndReadUserObjectI(CFE_TBL_FileDef_t *localDef, boolean *DataIsAllZeros, int32 *objSymbolIndex)
{
    ...
    i = LocateAndReadUserObjectIndex(localDef->ObjectName, FALSE);
    
    if ((i == FAILED) || (i == NumSymbols))
    {
        printf("Error! Unable to find '%s' object in '%s'\n", localDef->ObjectName, SrcFilename);
        Status = FAILED;
    }
    else
    {
        if (Verbose) printf("Found '%s' object as Symbol #%d\n", localDef->ObjectName, (i+1));
        *objSymbolIndex = i;
        
        if (strcmp(SectionNamePtrs[SymbolPtrs[*objSymbolIndex]->st_shndx], ".bss") == 0)
        {
            ...
            *DataIsAllZeros = TRUE;
            ...
            {
                printf("Object Data:\n");
                for (i=0; i<SymbolPtrs[*objSymbolIndex]->st_size; i++)
                {
                    printf(" 0x%02x", 0);
                    j++;
                    if (j==16)
        ...
        else
        {
        	*DataIsAllZeros = FALSE;
            ...
            SeekOffset = SectionHeaderPtrs[SymbolPtrs[*objSymbolIndex]->st_shndx]->sh_offset +
                         (uint32)SymbolPtrs[*objSymbolIndex]->st_value;
            fseek(SrcFileDesc, SeekOffset, SEEK_SET);
        
            /* Determine if the elf file contained the size of the object */
            if (SymbolPtrs[*objSymbolIndex]->st_size != 0)
            {
                /* Check to see if the size in the elf file agrees with the size specified in our table def structure */
                if (SymbolPtrs[*objSymbolIndex]->st_size != localDef->ObjectSize)
                {
                    printf("ELF file indicates object '%s' is of size %d but table definition structure indicates size %d.\n",
                           localDef->ObjectName, SymbolPtrs[*objSymbolIndex]->st_size, localDef->ObjectSize);
                    if (localDef->ObjectSize < SymbolPtrs[*objSymbolIndex]->st_size)
                    {
                        SymbolPtrs[*objSymbolIndex]->st_size = localDef->ObjectSize;
                    }
                           
                    printf("Size of %d is assumed.\n", SymbolPtrs[*objSymbolIndex]->st_size);
                    Status = SIZEMISMATCH;
                }
            }
            else
...
int32 OutputDataToTargetFile()
{
	...
	/* Check for SC table hook. */
	scind = LocateAndReadUserObjectIndex(SC_ELF2TBL_HOOK, TRUE);
	if (scind == FAILED)
	{

		if (Verbose)
			printf(
					"\nDidn't match ObjectName 'SC_ELF2TBL_HOOK'... (length %d), so skipping special handling.\n",
					strlen(SC_ELF2TBL_HOOK));

		/* Output the data from the object file (standard table handling) */
	...
	{
		/* Special handling for the SC tables.
		 * Writes to DstFileDesc. */
		if (Verbose)
			printf("\nMatched ObjectName %s... (length %d).\n", SC_ELF2TBL_HOOK,
					strlen(SC_ELF2TBL_HOOK));
		Status = GenerateSCTable();
	}
...
/*
 * Locate a symbol name in the symbol table, return the index or FAILED.
 * Args: object name string (must be null-terminated)
 *       boolean, T=don't put out a warning if the symbol is missing (this is
 *                  a known outcome) but still return FAILED,
 *                F=always put out a warning if the symbol is missing
 */
int32 LocateAndReadUserObjectIndex(char * objName, boolean doWarn)
{
    int32 i = 0;
    ...
    /* Search the symbol table for the user defined object */
    if (Verbose)
        printf("\nTrying to match ObjectName '%s'... (length %d)", objName,
                strlen(objName));

    while (i < NumSymbols)
    {
        if (Verbose)
            printf("\nSymbol Search loop %d: SymbolName ='%s' ", i,
                    SymbolNames[i]);

        /* Check to see if the symbol names match as far as the ObjectName is defined */
        if (strncmp(SymbolNames[i], objName, strlen(objName)) == 0)
        {
            if (Verbose)
                printf("Found ObjectName '%s' inside SymbolName '%s'\n",
                        objName, SymbolNames[i]);

            /* Check to see if the Symbol Name happens to have one extra character */
            if ((strlen(SymbolNames[i]) - strlen(objName)) == 1)
            {
                if (Verbose)
            ...
            else if ((strlen(SymbolNames[i]) - strlen(objName)) == 0)
            {
                if (Verbose)
                    printf("\nFound an exact match! Symbol='%s' Object='%s'\n",
                            SymbolNames[i], objName);
                break;
            }
        }
        ...
        if (Verbose)
        {
            printf("strstr[%d] = %d; strlenSN = %d; strlenON = %d\n", i,
                    strstr(SymbolNames[i], objName), strlen(SymbolNames[i]),
                    strlen(objName));
        }
    ...
    if (Verbose)
    {
        ...
        if (i < NumSymbols)
        {
            ...
            printf("\nSymbolName length = %d, ObjectName length = %d\n",
                    strlen(SymbolNames[i]), strlen(objName));
        }
    }
...
/*
 * Retrieve the raw bytes from the designated symbol.
 * Args:
 * 		dataDescrip: The data to access (required: ObjectName & ObjectSize)
 * 		data: a buffer of at least size dataDescrip.ObjectSize
 * Returns:
 * 		SUCCESS = symbol found and data populated (by zeros if symbol
 * 			was in the .bss),
 * 		FAILED = symbol not found or there was a size problem
 */
int32 GetBytesForSymbol(CFE_TBL_FileDef_t *dataDescrip, uint8 *data)
{

    boolean DataIsAllZeros = FALSE;
    int32 status = LocateAndReadUserObject(dataDescrip, &DataIsAllZeros);
    size_t readbytes = 0;
    ...
    else
    {
        if (DataIsAllZeros)
        {
            memset((void *) data, 0, dataDescrip->ObjectSize);
        }
        else
        {
            readbytes = fread((void *) data, 1U, dataDescrip->ObjectSize,
                    SrcFileDesc);
            if (readbytes != dataDescrip->ObjectSize)
            {
                if (Verbose)
                    printf(
                            "\n Error: Failed to read expected bytes from SrcFileDesc.\n");
                status = FAILED;
            }
        }
    }
...
uint8 CCSDS_ComputeCheckSum(CCSDS_CmdPkt_t *PktPtr)
{
    ...
    /* Host & Target could have an endian mismatch.  So pull the uint16
     * then decide to byteswap, if needed. (length is always BE) */
    memcpy((void *) (&PktLen), (void *) &(PktPtr->PriHdr.Length),
            sizeof(uint16));

    if (ThisMachineIsLittleEndian)
    {
        SwapUInt16(&PktLen);
    }
    PktLen += 7; /* see ccsds.h */
...
/******************************************************************************
 **  Function:  Write_CCSDS_Checksum()
 **
 **  Purpose:
 **    Write the checksum into the message, taking into account the target
 **    vs host endianness.
 **
 **  Arguments:
 **    PktPtr   : Pointer to header of command packet.
 **               The checksum field in the packet will be modified.
 **    cksum    : the checksum to write
 **
 **  Return:
 **    (none)
 */
void Write_CCSDS_Checksum(CCSDS_CmdPkt_t *PktPtr, uint8 cksum)
{
    ...
    if (ByteSwapRequired)
    {
        /* Different?  The handle it so that its proper for the target. */
        memcpy((void *) &tmp, (void *) PktPtr, sizeof(CCSDS_CmdPkt_t));
        SwapUInt16(&(tmp.SecHdr.Command));
        CCSDS_WR_CHECKSUM(tmp.SecHdr, cksum);
        SwapUInt16(&(tmp.SecHdr.Command));
        memcpy((void *) PktPtr, (void *) &tmp, sizeof(CCSDS_CmdPkt_t));
    }
    else
    {
        /* same, then use the ccsds.h macro */
        CCSDS_WR_CHECKSUM(PktPtr->SecHdr, cksum);
    }
}
...
void CCSDS_LoadCheckSum(CCSDS_CmdPkt_t *PktPtr)
{
    ...
    Write_CCSDS_Checksum(PktPtr, 0);

    /* Compute and load new checksum. */
    CheckSum = CCSDS_ComputeCheckSum(PktPtr);

    printf("\nCalculated checksum is: 0x%08x.\n",CheckSum);

    Write_CCSDS_Checksum(PktPtr, CheckSum);

} /* END CCSDS_LoadCheckSum() */

/*
 * Handle the generation of a SC table with a special object name marker.
 */
int32 GenerateSCTable(void)
{
    ...
    uint8 tabletype = 0; /* 0=ATS, 1=RTS */
    uint16 packetMinSize = 0; /* per-message limit */
    uint16 packetMaxSize = 0; /* per-message limit */
    uint16 numCmds = 0; /* Number of commands to assemble into the table */
    /* local set of commands, note: will convert values to native for use */
    RTS_List_Entry RTSCommandList[SC_MAX_CMDS];
    ATS_List_Entry ATSCommandList[SC_MAX_CMDS];
    const uint8 ZeroByte = 0;

    CFE_TBL_FileDef_t symbol;
    uint16 i, j;
    ...
    uint16 byteCount = 0;
    uint8 *msgBuffer;
    uint16 atsSentinelCheck = 0;
    uint16 atsCmdNum;

    printf("\nBeginning SC table special handling.\n");

    /* SCTableType */
    if (Verbose)
        printf("\n SC table special handling: SCTableType.\n");
    strncpy(symbol.ObjectName, "SCTableType", CFETBLSYMLENGTH);
    symbol.ObjectSize = sizeof(uint8);
    status = GetBytesForSymbol(&symbol, &tabletype);
    if (status == FAILED)
    {
        printf("\nError getting SCTableType.  Aborting.\n");
        return status;
    }

    if (tabletype == 0)
    { /* ATS */
        printf("\nSC table special handling: ATS table detected.\n");
    }
    else if (tabletype == 1)
    {
        printf("\nSC table special handling: RTS table detected.\n");
    }
    else
    {
        printf("\nSCTableType found, but illegal value.\n");
        return FAILED;
    }

    /* SCPacketMinSize */
    if (Verbose)
        printf("\n SC table special handling: SCPacketMinSize.\n");
    strncpy(symbol.ObjectName, "SCPacketMinSize", CFETBLSYMLENGTH);
    symbol.ObjectSize = sizeof(uint16);
    status = GetBytesForSymbol(&symbol, (uint8 *) &packetMinSize);

    if (status == FAILED)
    {
        printf("\nError getting SCPacketMinSize.  Aborting.\n");
        return status;
    }

    if (ByteSwapRequired)
    {
        SwapUInt16(&packetMinSize);
    }

    if (Verbose)
        printf("\nSC table: SCPacketMinSize = %d.\n", packetMinSize);

    /* SCPacketMaxSize */
    if (Verbose)
        printf("\nSC table special handling: SCPacketMaxSize.\n");
    strncpy(symbol.ObjectName, "SCPacketMaxSize", CFETBLSYMLENGTH);
    symbol.ObjectSize = sizeof(uint16);
    status = GetBytesForSymbol(&symbol, (uint8 *) &packetMaxSize);

    if (status == FAILED)
    {
        printf("\nError getting SCPacketMaxSize.  Aborting.\n");
        return status;
    }

    if (ByteSwapRequired)
    {
        SwapUInt16(&packetMaxSize);
    }

    if (Verbose)
        printf("\nSC table: SCPacketMaxSize = %d.\n", packetMaxSize);

    /* SCCommandList_NUM_CMDS */
    if (Verbose)
        printf("\nSC table special handling: SCCommandList_NUM_CMDS.\n");
    strncpy(symbol.ObjectName, "SCCommandList_NUM_CMDS", CFETBLSYMLENGTH);
    symbol.ObjectSize = sizeof(uint16);
    status = GetBytesForSymbol(&symbol, (uint8 *) &numCmds);

    if (status == FAILED)
    {
        printf("\nError getting SCCommandList_NUM_CMDS.  Aborting.\n");
        return status;
    }
    ...
    printf("\nSC table: SCCommandList_NUM_CMDS = %d.\n", numCmds);

    if (numCmds >= SC_MAX_CMDS)
    {
        printf("\nSC table: SCCommandList_NUM_CMDS too large.  Aborting.\n");
        printf("(Fix elf2cfetbl hardcoded assumption.)\n");
        return FAILED;
    }

    /* SCCommandList */
    if (Verbose)
        printf("\nSC table special handling: SCCommandList.\n");
    strncpy(symbol.ObjectName, SC_ELF2TBL_HOOK, CFETBLSYMLENGTH);
    if (tabletype == 0) /* ATS */
    {
        symbol.ObjectSize = sizeof(ATS_List_Entry) * numCmds;
        status = GetBytesForSymbol(&symbol, (uint8 *) &ATSCommandList);
    }
    else
    {
        symbol.ObjectSize = sizeof(RTS_List_Entry) * numCmds;
        status = GetBytesForSymbol(&symbol, (uint8 *) &RTSCommandList);
    }

    if (status == FAILED)
    {
        printf("\nError getting SCCommandList.  Aborting.\n");
        return status;
    }
    else
    {
        if (Verbose)
            printf("\nSuccessfully loaded SCCommandList.\n");
    }

    /* convert data to host convention, if necessary */
    if (ByteSwapRequired)
    {
        for (i = 0; i < numCmds; i++)
        {
            if (tabletype == 0) /* ATS */
            {
                SwapUInt32(&(ATSCommandList[i].TimeTag));
                SwapUInt16(&(ATSCommandList[i].byteSize));
            }
            else
            {
                SwapUInt16(&(RTSCommandList[i].TimeTag));
                SwapUInt16(&(RTSCommandList[i].byteSize));
            }
        }
    }

    msgBuffer = (uint8 *) malloc(packetMaxSize + 1);
    if (msgBuffer == NULL)
    {
        printf("\nError: SC message buffer malloc fail.  Aborting.\n");
        return FAILED;
    }
    /* Note, after this point don't return without freeing msgBuffer. */

    for (i = 0; i < numCmds; i++)
    {

        // Place ATS command number into table (write to file)
        if (tabletype == 0) /* ATS */
        {
            atsCmdNum = i + 1; /* 1-based */

            byteCount += 2;
            if (byteCount > TblFileDef.ObjectSize)
            {
                printf("\nError: Exceeded table size.  Aborting.\n");
                status = FAILED;
                break;
            }
            if (Verbose)
            ...
            if (ByteSwapRequired)
            {
                SwapUInt16(&atsCmdNum);
            }
            wsz = fwrite(&atsCmdNum, sizeof(uint16), 1, DstFileDesc);
            if (wsz != 1)
            {
                printf(
                        "\nError: timetag write to table file failed.  Aborting.\n");
                status = FAILED;
                break;
            }
        }

        // place time into table (write to file)
        if (tabletype == 0) /* ATS */
        {
            atsTimeTag = ATSCommandList[i].TimeTag; // host convention (from above)
            ...
            /* Note, ATS time is a platform-specific uint32, held as two uint16s.
             * The high/lo bits are not placed separately. */

            if (ByteSwapRequired)
            {
                SwapUInt32(&atsTimeTag); // back to target
            }

            wsz = fwrite(&atsTimeTag, 1, 4, DstFileDesc);
            if (wsz != 4)
            {
                printf(
                        "\nError: ATS timetag write to table file failed.  Aborting.\n");
                status = FAILED;
                break;
            }
        }
        ...
        {
            rtsTimeTag = RTSCommandList[i].TimeTag; // host convention (from above)
            ...
            if (ByteSwapRequired)
            {
                SwapUInt16(&rtsTimeTag); // back to target
            }

            wsz = fwrite(&rtsTimeTag, sizeof(uint16), 1, DstFileDesc);
            if (wsz != 1)
            {
                printf("\nError: RTS timetag write to table file failed.  Aborting.\n");
                status = FAILED;
                break;
            }
        }

        // -get command from list, check size & put in buffer
        // -compare command object size to reported msg size
        memset(msgBuffer, 0, packetMaxSize + 1);
        if (tabletype == 0) /* ATS */
        {
            strncpy(symbol.ObjectName, ATSCommandList[i].cmdSymbolName, CFETBLSYMLENGTH);
            symbol.ObjectSize = ATSCommandList[i].byteSize;
        }
        else
        {
            strncpy(symbol.ObjectName, RTSCommandList[i].cmdSymbolName, CFETBLSYMLENGTH);
            symbol.ObjectSize = RTSCommandList[i].byteSize;
        }

        /* Check message size reported through the command list
         * object file table so we don't blow our buffer. */
        if (symbol.ObjectSize > packetMaxSize)
        {
            printf(
                    "\nError: Command %d sizeof() is too large.  Aborting.\n",
                    i);
            status = FAILED;
            break;
        }

        status = GetBytesForSymbol(&symbol, msgBuffer);
        if (status == FAILED)
        {
            printf(
                    "\nError: failed to load command %d with symbol %s.  Aborting.\n",
                    i, symbol.ObjectName);
            break;
        }

        /* Check min msg size
         * Note, there is the size from the object file table and there is the reported
         * size in the mssage primary header - check both. */
        if ((symbol.ObjectSize < packetMinSize)
                || (CCSDS_RD_LEN(*(CCSDS_PriHdr_t*) (msgBuffer)) < packetMinSize))
        {
            printf(
                    "\nError: Message size in command %d is too small.  Aborting.\n",
                    i);
            status = FAILED;
            break;
        }

        /* Check max msg size reported in the mssage primary header */
        if (CCSDS_RD_LEN(*(CCSDS_PriHdr_t*) (msgBuffer)) > packetMaxSize)
        {
            printf(
                    "\nError: Message size in command %d is too large.  Aborting.\n",
                    i);
            status = FAILED;
            break;
        }

        // Now that length checks are satisfied, pad in msgBuffer for uint16 alignment.

        if ((symbol.ObjectSize % 2) == 1)
        {
            symbol.ObjectSize++;
        }
        byteCount += symbol.ObjectSize;
        ...
        if (Verbose)
            printf("\nLoaded command %d with symbol %s (%d bytes, padded).\n",
                    i, symbol.ObjectName, byteCount);

        // Check message data - secondary header
        if (CCSDS_RD_SHDR(*(CCSDS_PriHdr_t *) msgBuffer) != 1)
        {
            printf(
                    "\nError: command %d with symbol %s doesn't specify a secondary header.  Aborting.\n",
                    i, symbol.ObjectName);
            status = FAILED;
            break;
        }
        if (Verbose)
            printf("\nCommand %d with symbol %s has a secondary header.\n", i,
                    symbol.ObjectName);

        // Check message data - command msg
        if (CCSDS_RD_TYPE(*(CCSDS_PriHdr_t *) msgBuffer) != 1)
        {
            printf(
                    "\nError: command %d with symbol %s isn't a command packet type.  Aborting.\n",
                    i, symbol.ObjectName);
            status = FAILED;
            break;
        }
        if (Verbose)
            printf("\nCommand %d with symbol %s is a command packet type.\n", i,
                    symbol.ObjectName);

        // calc msg checksum
        CCSDS_LoadCheckSum((CCSDS_CmdPkt_t *) msgBuffer);

        // -place cmd in table (write to file)
        wsz = fwrite(msgBuffer, 1, symbol.ObjectSize, DstFileDesc);
        if (wsz != symbol.ObjectSize)
        {
            printf("\nError: command write to table file failed.  Aborting.\n");
            status = FAILED;
            break;
        }
        if (Verbose)
            printf("\nWrote command %d, size: %d.\n", i, symbol.ObjectSize);

    }

    /* Continue to write the rest of the bytes for the table as zeros. */
    if (status != FAILED)
    {
        for (; byteCount < SymbolPtrs[UserObjSymbolIndex]->st_size; byteCount++)
        {
            if (tabletype == 0)
            { /* ATS */
                atsSentinelCheck++;
            }
            fwrite(&ZeroByte, 1, 1, DstFileDesc);
        }

        if (tabletype == 0)
        { /* ATS */
            if (atsSentinelCheck < 7)
            {
                printf(
                    "\nError: Not enough space remaining in table for ATS sentinel word.  Aborting.\n");
                status = FAILED;
            }
        }
    }
    ...
    printf("\nCompleted SC table special handling.\n");

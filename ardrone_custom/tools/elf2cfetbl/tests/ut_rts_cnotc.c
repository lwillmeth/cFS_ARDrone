...
**  agreement.
**
**  elf2cfetbl test: message has a secondary header but isn't flagged as a command
**
...
static CFE_TBL_FileDef_t CFE_TBL_FileDef RETAIN_SYMBOL = {
		...
		"ut_rts_cnotc.tbl", (SC_RTS_BUFF_SIZE * sizeof(uint16)) };
...
/* appID=0x22, no 2ndary header, 0x0800=secondary header present, ccsds version=0 
   but not flagged as a command */
#define MID_WITH_NO_CMDFLAG (0x0022 & 0x0800)

static CFE_SB_CmdHdr_t MsgNotACommand RETAIN_SYMBOL = { {
/* primary message header */
{ INPLACE_BE_HI(CFE_MAKE_BIG16(MID_WITH_NO_CMDFLAG)),
  INPLACE_BE_LO(CFE_MAKE_BIG16(MID_WITH_NO_CMDFLAG)) }, /* MID */
{ INPLACE_BE_HI(CMD_FLAGS_BE), INPLACE_BE_LO(CMD_FLAGS_BE) }, /* Seq/Flags */
{ INPLACE_BE_HI(CMD123_SIZE_BE), INPLACE_BE_LO(CMD123_SIZE_BE) } /* Length */
},
/* secondary command header */
...
static RTS_List_Entry SCCommandList[NUM_CMDS] RETAIN_SYMBOL =
	{ { 0, "sc_noop", sizeof(sc_noop) },
	  ...
	  { 5, "MsgNotACommand", sizeof(MsgNotACommand) } };

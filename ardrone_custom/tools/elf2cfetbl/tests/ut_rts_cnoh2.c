...
**  agreement.
**
**  elf2cfetbl test: msg has no secondary header (not a command)
**
...
static CFE_TBL_FileDef_t CFE_TBL_FileDef RETAIN_SYMBOL = {
		...
		"ut_rts_cnoh2.tbl", (SC_RTS_BUFF_SIZE * sizeof(uint16)) };
...
/* appID=0x22, no 2ndary header, 0x1000=CMD type, ccsds version=0 */
#define MID_WITH_NO_HEADERFLAG (0x0022 & 0x1000)

static CFE_SB_CmdHdr_t MsgWithNo2ndHeader RETAIN_SYMBOL = { {
/* primary message header */
{ INPLACE_BE_HI(CFE_MAKE_BIG16(MID_WITH_NO_HEADERFLAG)),
  INPLACE_BE_LO(CFE_MAKE_BIG16(MID_WITH_NO_HEADERFLAG)) }, /* MID */
{ INPLACE_BE_HI(CMD_FLAGS_BE), INPLACE_BE_LO(CMD_FLAGS_BE) }, /* Seq/Flags */
{ INPLACE_BE_HI(CMD123_SIZE_BE), INPLACE_BE_LO(CMD123_SIZE_BE) } /* Length */
},
/* secondary command header */
...
static RTS_List_Entry SCCommandList[NUM_CMDS] RETAIN_SYMBOL =
	{ { 0, "sc_noop", sizeof(sc_noop) },
	  ...
	  { 5, "MsgWithNo2ndHeader", sizeof(MsgWithNo2ndHeader) } };
